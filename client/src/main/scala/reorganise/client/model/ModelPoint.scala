...
case class ModelPoint[Model <: AnyRef, Point] (data: ModelR[Model, Point], circuit: Circuit[Model]) {
  def createReader (component: ReactComponentC.ReqProps[ModelPoint[Model, Point], _, _, _])
                   (implicit feq: diode.FastEq[_ >: Point]) = {
    val proxy = connect (feq)
    proxy (component (_))
  }

  def createReader (component: ReactComponentC.ReqProps[ModelPoint[Model, Point], _, _, _], children: ReactNode*)
                   (implicit feq: diode.FastEq[_ >: Point]) = {
    val proxy = connect (feq)
  ...
  def wrap[C] (compB: ModelPoint[Model, Point] => C)(implicit ev: C => ReactElement, feq: FastEq[_ >: Point]): C =
    compB (ModelPoint (data, circuit))
  ...
  def connect (implicit feq: FastEq[_ >: Point]): ReactComponentC.ReqProps[(ModelPoint[Model, Point]) => ReactElement, Point, _, TopNode] = {
    class Backend (t: BackendScope[ModelPoint[Model, Point] => ReactElement, Point]) {
      ...
      private def changeHandler (cursor: ModelR[Model, Point]): Unit =
    ...
    ReactComponentB[ModelPoint[Model, Point] => ReactElement] ("ModelPointWrapper")
      ...
      .shouldComponentUpdate (scope => feq.neqv (scope.currentState, scope.nextState))
      .build
      .set (js.undefined)
  }
}
